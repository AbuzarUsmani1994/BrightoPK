...
{
    public class GetPartiesRelatedToSegmentController : ApiController
    {
        ...
        public IHttpActionResult Get(int SegmentID,int RegionID,int CityID,int SOID)
        {
            ...
            try
            {
                ...
                if (SegmentID > 0)
                {
                    ...
                    var result = dbContext.Retailers.Where(x=>x.SegmentTypeID==SegmentID && x.RegionID== RegionID && x.SaleOfficerID==SOID && x.IsActive==true).OrderBy(x => x.ShopName).ToList();
